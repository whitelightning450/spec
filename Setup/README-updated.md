...
#####  IMU Tare
To perform the IMU’s accelerometer tare procedure, you will run the IMU/misc/tare_imu.py script.
- 1.	Open the tare_imu.py script and scroll down to line 59. Change the number of iterations from 100 to however many you’d like to run. The pitch and roll values bounce around, so the more iterations, the better the average value you will obtain. 

<img src="/app/static/images/imu_tare.jpg" alt="IMU tare code" width="200" /><br>
- Run the script: python tare_imu.py

That’s all you need to do. The tare values will be saved to the config.json file and will be read in and used each time the IMU is read to correct the pitch and roll values.
